...
exports.createManager = function (params, res) {
    MongoClient.connect(dbconn, (err, mongo) => {
        if (err) res({
            ...
            mongo.db(obj.db).collection(obj.manager).insert({
                                        ...
                                        if (event) {
                                            ...
                                            for (var i = 0; i < response.length; i++)
                                                for (var j = 0; j < response[i].schedules.length; j++) {
                                                    if (busyWeek[response[i].weekDay][response[i].schedules[j]] == week.operators.length) response[i].schedules.splice(j, 1);
                                                    else if (((busyMonth[response[i].weekDay][response[i].date + "T" + response[i].schedules[j] + ":00.000Z"] || 0) + (busyWeek[response[i].weekDay][response[i].schedules[j]] || 0)) == week.operators.length) response[i].schedules.splice(j, 1);
                                                }
                                            res({
                                                code: 0,
